...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // === CEO / Founder access (explicit CEO mode) ===
      if (showCEO) {
        if (!accessCode.trim()) {
      ...
      // === CEO code entered as password in normal login ===
  ...
  return (
    <div className="min-h-screen bg-gradient-luxury flex flex-col">
      ...
      <div className="p-4">
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-cream/50 hover:text-cream text-sm transition-colors"
        >
        ...
        <div className="w-full max-w-md">
          ...
          {isSignUp && !showCEO && (
            <div className="mb-5 flex flex-wrap justify-center gap-x-4 gap-y-1 text-xs text-cream/45">
              <span className="flex items-center gap-1">
                <Check className="w-3 h-3 text-gold" /> AI generates content
              </span>
              <span className="flex items-center gap-1">
                <Check className="w-3 h-3 text-gold" /> Auto-publish to 7 platforms
              </span>
              <span className="flex items-center gap-1">
                <Check className="w-3 h-3 text-gold" /> Cancel anytime
              </span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="glass-card p-6 sm:p-8 space-y-4">
            ...
            {showCEO ? (
              ...
              <>
                <div className="space-y-1.5">
                  <label className="text-xs text-cream/60 font-medium" htmlFor="email">
                    Email
                  </label>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-cream/60 font-medium" htmlFor="password">
                    Password
                  </label>
                  <div className="relative">
          ...
          {!showCEO && (
            <button
              ...
              className="mt-4 w-full flex items-center justify-center gap-2 text-xs text-cream/40 hover:text-gold transition-colors py-2"
            >
          ...
          <div className="mt-6 p-4 glass-card text-center">
            <p className="text-xs text-cream/50 mb-2">
              After your 7-day trial, plans start at{" "}
              <span className="font-semibold text-gold">$19/month</span>
            </p>
            <a
              href="https://buy.stripe.com/dRmcN51blcX24vreeecwg08"
              target="_blank"
