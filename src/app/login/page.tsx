...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CEO code in password field
      if (password.toUpperCase().trim() === "AUREA2026") {
        const res = await fetch("/api/auth/founder", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ code: "AUREA2026" }),
        });
        const data = await res.json();
        if (!res.ok) {
          setError(data.message || "Invalid founder code.");
      ...
      if (isSignUp) {
        ...
        const data = await res.json();
        if (!res.ok) { setError(data.message || "Could not create account."); setLoading(false); return; }
  ...
  return (
    <div className="min-h-screen bg-gradient-luxury flex flex-col">
      <div className="p-4">
        <Link href="/" className="inline-flex items-center gap-1.5 text-cream/50 hover:text-cream text-sm transition-colors">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-cream font-heading">
              {isSignUp ? "Start Your Free Trial" : "Welcome Back"}
            </h1>
            <p className="text-sm text-cream/50 mt-2">
              {showCEO ? "Enter your founder access code" : isSignUp ? "7 days free · No credit card required" : "Sign in to your account"}
            </p>
          </div>
          {isSignUp && !showCEO && (
          ...
          <form onSubmit={handleSubmit} className="glass-card p-6 sm:p-8 space-y-4">
            ...
            {showCEO ? (
              <div className="space-y-1.5">
                <label className="text-xs text-cream/60 font-medium flex items-center gap-1.5" htmlFor="code"><Crown className="w-3.5 h-3.5 text-gold" /> Founder Access Code</label>
                <input id="code" type="text" placeholder="Enter your code" value={accessCode} onChange={e => setAccessCode(e.target.value)} className="input-luxury" autoFocus />
              </div>
            ) : (
              <>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input id="password" type={showPassword ? "text" : "password"} placeholder={isSignUp ? "At least 8 characters" : "Enter your password"} value={password} onChange={e => setPassword(e.target.value)} className="input-luxury pl-10 pr-10" />
            ...
            <button type="submit" className="btn-gold w-full text-sm py-3" disabled={loading}>
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
                  {showCEO ? "Verifying..." : isSignUp ? "Creating account..." : "Signing in..."}
                </span>
            ...
            {!showCEO && (
              <div className="flex items-center gap-2 p-2.5 rounded-lg bg-gold/5 border border-gold/10">
                ...
                <p className="text-[10px] text-cream/50">CEO? Use <span className="font-semibold text-gold">AUREA2026</span> as your password</p>
              </div>
            )}
            <div className="text-center text-xs text-cream/40">
              {showCEO ? (
                <button type="button" onClick={() => setShowCEO(false)} className="text-gold hover:text-gold-light font-medium transition-colors">Back to regular login</button>
              ) : isSignUp ? (
          ...
          <div className="mt-6 p-4 glass-card text-center">
            ...
            <a href="https://buy.stripe.com/dRmcN51blcX24vreeecwg08" target="_blank" rel="noopener noreferrer">
              <button className="btn-outline text-xs py-2 px-6">Upgrade Now</button>
            </a>
          </div>
        </div>
      </div>
