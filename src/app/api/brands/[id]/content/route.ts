...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      execSync(`team-db "INSERT INTO content_items (id, brand, prompt, script, captions, hashtags, status) VALUES ('${contentId}', '${brandId.replace(/'/g, "''")}', '${body.prompt.replace(/'/g, "''")}', '${(result.script || "").replace(/'/g, "''")}', '${JSON.stringify(result.captions || {}).replace(/'/g, "''")}', '${JSON.stringify(result.hashtags || {}).replace(/'/g, "''")}', 'generated')"`, { encoding: "utf8" });
    } catch (e) { contentId = `cnt_${Date.now()}`; }

    if (body.schedule) {
      const { generateSchedule } = await import("@/lib/services/backend");
      result.schedule = generateSchedule({ brandName: brandId, platforms: body.schedule.platforms || ["tiktok", "instagram"], postsPerDay: body.schedule.postsPerDay || 3, startDate: new Date(), durationDays: body.schedule.durationDays || 7 });
    }

    return NextResponse.json({ success: true, brandId, content: result, contentId }, { status: 201 });
  } catch (e) {
