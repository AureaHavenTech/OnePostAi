...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      execSync(`team-db "INSERT INTO content_items (id, brand, prompt, script, captions, hashtags, status) VALUES ('${contentId}', '${brandId.replace(/'/g, "''")}', '${body.prompt.replace(/'/g, "''")}', '${(JSON.stringify(r.platformContent || []).replace(/'/g, "''"))}', '${JSON.stringify(r.captions || {}).replace(/'/g, "''")}', '${JSON.stringify(r.hashtags || {}).replace(/'/g, "''")}', 'generated')"`, { encoding: "utf8" });
